...
export async function buildDocx(d) {
  ...
  // Skills — flat "Label: items" lines (matches uploaded resume)
  const skills = (d.skills || []).filter((g) => (g.items || []).length);
  if (skills.length) {
    children.push(heading("Relevant Skills & Certifications"));
    for (const g of skills) {
      children.push(new Paragraph({
        spacing: { after: 60 },
        children: [
          new TextRun({ text: `${g.label}: `, bold: true, size: 20, font: "Calibri", color: INK }),
          new TextRun({ text: (g.items || []).join(", "), size: 20, font: "Calibri", color: INK }),
        ],
      }));
    }
  }
